...
export function MGroups() {
  ...
  return (
    <div className="px-5 py-5">
      ...
      {groups.map(g => (
        <button key={g.id} onClick={() => navigate(`/member/group/${g.id}`)} className="w-full text-left bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl p-4 shadow-sm mb-4 hover:shadow-md hover:border-emerald-200 dark:hover:border-emerald-800 transition-all">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4 text-center">
            <div className="bg-gray-50 dark:bg-muted rounded-xl p-2.5">
              <p className="text-sm font-bold text-gray-900 dark:text-white">{formatNaira(g.savingsBalance)}</p>
