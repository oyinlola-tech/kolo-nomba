    ...
    <div>
      ...
      <div className="space-y-3">
        {notifications.map((n, i) => (
          <Card key={n.id || i} className={`p-4 flex items-start gap-4 ${!n.read ? "border-l-2 border-l-emerald-500" : ""}`}>
            ...
            <div className="flex-1">
              <p className={`text-sm font-semibold ${n.read ? "text-gray-600 dark:text-gray-400" : "text-gray-900 dark:text-white"}`}>{n.title}</p>
              <p className="text-xs text-gray-500 dark:text-muted-foreground mt-0.5">{n.body}</p>
